...
4. **更新文档**：
   ...
   - 创建该包的 AGENTS.md 文档（面向 AI 开发维护）
   - 如有必要，创建 `skills/` 目录下的最佳实践 skill（面向 AI 辅助用户使用）
5. **更新依赖**：运行 `composer script:update-packages` 更新根 composer.json

## 文档规范

每个包目录下维护三类文档，职责严格分离，不得混用：

### README.md — 面向用户，介绍包的功能
- **受众**：使用该包的开发者（人读）
- **内容**：包的功能介绍、安装方式、基本用法示例、API 概览
- **不应包含**：最佳实践建议、推荐/不推荐写法、开发规范、测试说明

### AGENTS.md — 面向 AI，辅助包的开发维护
- **受众**：协助开发/维护该包的 AI（AI 读）
- **内容**：代码结构、目录说明、工作流程、开发规范、测试方式、注意事项
- **不应包含**：面向用户的使用教程、最佳实践建议

### skills/ — 面向 AI，辅助用户正确使用包
- **受众**：协助用户使用该包的 AI（AI 读）
- **内容**：推荐写法（有立场，给出明确建议）、常见坑、典型场景示例
- **不应包含**：纯功能描述、API 文档、开发规范
- **核心原则**：有立场，说"推荐这样做"而不是"可以这样也可以那样"

> 如果 README 已经包含完整的最佳实践，skills 可以不写（避免重复）。
